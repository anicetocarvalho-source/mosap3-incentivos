-- Atualiza funções do dashboard para excluir produtores soft-deleted (status = 'Removido')
-- mantendo coerência com a página /patec e demais módulos.
...
BEGIN
  WITH scoped AS (
    ...
    FROM farmers
    WHERE COALESCE(status, '') <> 'Removido'
      AND (p_scope = 'global'
  ...
  ELSIF p_scope = 'province' THEN
    ...
    FROM farmers
    WHERE COALESCE(status,'') <> 'Removido'
      AND province = ANY(p_provinces) AND school IS NOT NULL AND school <> '';
  ELSE
