...
const RecentSales = ({ supplierId }: { supplierId: string }) => {
  ...
  useEffect(() => {
    supabase.from("pos_sales").select("*").eq("supplier_id", supplierId).order("created_at", { ascending: false }).limit(10)
      .then(({ data }) => setSales(data || []));
  }, [supplierId]);

  if (sales.length === 0) return <p className="text-sm text-muted-foreground py-4 text-center">Nenhuma venda registada</p>;
